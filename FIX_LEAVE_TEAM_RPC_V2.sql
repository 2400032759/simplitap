...
-- UPDATED: Handles multiple profiles/cards per user
...
DECLARE
  ...
  v_email text;
BEGIN
  -- Get current user's team and email from ANY of their profiles
  -- We use LIMIT 1 because all profiles for a user should belong to the same team (or none)
  ...
  -- We match by team_id and email
  UPDATE public.team_members
  SET status = 'left', updated_at = NOW()
  WHERE team_id = v_team_id 
  AND (email = v_email OR email IS NOT DISTINCT FROM v_email);

  -- Update ALL profiles for this user to remove team association
